...
- The same exact plan bytes execute at most once per database. Completed executions reject duplicate replay with exit 2 and reprint the stored result; interrupted or failed executions resume, skipping already-complete children. The resolved execution root (`--output-dir/<plan-sha256>`) is persisted as immutable execution identity, so replay or resume must use the original `--output-dir`; changing it is rejected fail-closed rather than treated as a location-independent replay.
